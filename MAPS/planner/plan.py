...
from MAPS.core.stage import InputSource, InputSourceKind, Stage, StageInputBinding, StageOutputBinding
...
def _input_source(tensor_id: int, transition_id: int | None) -> InputSource:
    if transition_id is None:
        return InputSource(
            kind=InputSourceKind.EXTERNAL,
            external_base_addr=tensor_id + 1,
        )
    return InputSource(
        kind=InputSourceKind.TRANSITION,
