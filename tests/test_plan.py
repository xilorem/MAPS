...
from MAPS.core.stage import ExternalInput, LocalInput, TransitionInput
...
def test_build_pipeline_from_graph_assembles_stages_transitions_and_bindings() -> None:
    ...
    assert isinstance(pipeline.stages[0].inputs[0].source, ExternalInput)
    assert isinstance(pipeline.stages[1].inputs[0].source, TransitionInput)
...
def test_build_pipeline_from_graph_uses_local_input_for_identical_layouts() -> None:
    mesh = magia_mesh()
    ...
    plan0 = StagePlan(
        ...
        tile_count=2,
        logical_shape=(2, 1),
        input_layouts=gemm0.default_input_layouts(submesh, logical_shape=(2, 1)),
        output_layouts=gemm0.default_output_layouts(submesh, logical_shape=(2, 1)),
    )
    plan1 = StagePlan(
        stage_id=1,
        tile_count=2,
        logical_shape=(2, 1),
        input_layouts=gemm1.default_input_layouts(submesh, logical_shape=(2, 1)),
        output_layouts=gemm1.default_output_layouts(submesh, logical_shape=(2, 1)),
    )

    pipeline = _build_pipeline_from_graph(graph, mesh, {0: plan0, 1: plan1})

    assert pipeline.transitions == ()
    source = pipeline.stages[1].inputs[0].source
    assert isinstance(source, LocalInput)
    assert source.stage_id == 0
    assert source.output_idx == 0
...
def test_build_pipeline_parses_balances_maps_and_builds_transitions() -> None:
    ...
    if pipeline.transitions:
        ...
        assert isinstance(pipeline.stages[1].inputs[0].source, LocalInput)
    assert isinstance(pipeline.stages[0].inputs[0].source, ExternalInput)
